...
public class BeanUtils {
	...
	public static void copy(Object source, Object target) {
		try {
			...
			BeanInfo targetBeanInfo = Introspector.getBeanInfo(target
					.getClass());
			// 根据getter来判断有哪些属性
			PropertyDescriptor[] targetProps = targetBeanInfo
					.getPropertyDescriptors();
			
			for (PropertyDescriptor targetProp : targetProps) {
				try {
					// 获取source与target相同name的属性
					PropertyDescriptor sourceProp = getPropertyDescriptor(
							source.getClass(), targetProp.getName());
					...
					if (targetWriteMethod == null) {
						...
						MethodDescriptor methodDesc = getMethodDescriptor(
								target.getClass(), SETTER_PREFIX
										+ targetProp.getName().substring(0, 1)
												.toUpperCase()
					...
					if (targetWriteMethod != null && sourceProp != null
							&& sourceProp.getReadMethod() != null) {
						Object value = getValue(sourceProp.getReadMethod(),
								source, new Object[0]);
						if (value != null) {
							// 判断getter与setter的参数类型是否匹配，否则容易报IllegalArgumentException:
							// argument type mismatch
							if (value.getClass() == targetWriteMethod
									.getParameterTypes()[0]) {
								setValue(targetWriteMethod, target,
										new Object[] { value });
							} else if (value.getClass().isPrimitive()
									|| targetWriteMethod.getParameterTypes()[0]
											.isPrimitive()) {
								setValue(targetWriteMethod, target,
										new Object[] { value });
							}
	...
	private static Object getValue(Method method, Object obj, Object[] args)
	...
	private static void setValue(Method method, Object obj, Object[] args)
			throws IllegalArgumentException, IllegalAccessException,
			InvocationTargetException {
		if (method == null || obj == null) {
			logger.info("Method method, Object obj must not be null");
	...
	private static MethodDescriptor getMethodDescriptor(Class<?> clz,
	...
	private static PropertyDescriptor getPropertyDescriptor(Class<?> clz,
